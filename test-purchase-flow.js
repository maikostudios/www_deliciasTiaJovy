...
async function testPurchaseFlow() {
  ...
  try {
    ...
    products.forEach((p) => {
      console.log(
        `   - ${p.name} (${p.productType}) - $${p.price.toLocaleString()}`
      );
    });

    // 2. Seleccionar una torta configurable para testing
    console.log("\n🎂 STEP 2: Selecting Configurable Cake");
    const configurableCake = products.find(
      (p) => p.productType === "configurable"
    );
    ...
    console.log(
      `   - Available sizes: ${
        configurableCake.cakeConfig?.availableSizes?.join(", ") ||
        "Not configured"
      }`
    );
    ...
    const cakeConfiguration = {
      ...
      specialInstructions:
        "Por favor usar stevia en lugar de azúcar. Decoración con tema de cumpleaños.",
    };
    ...
    const simpleProduct = products.find(
      (p) => p.productType === "simple" && p.category === "panaderia"
    );
    ...
    const orderData = {
      ...
      total: cartTotal,
      comments:
        "Entrega preferida el sábado en la tarde. Por favor confirmar horario por WhatsApp. Pago 50% adelantado por transferencia.",
      status: "pending",
      createdAt: new Date(),
      updatedAt: new Date(),
    };
    ...
    console.log(
      `   - Customer ID: ${customerData.email
        .replace("@", "_")
        .replace(".", "_")}`
    );
