    ...
    <div class="hidden md:flex md:w-64 md:flex-col">
      <div class="flex flex-col flex-grow pt-5 overflow-y-auto bg-white border-r border-gray-200">
        ...
        <div class="flex items-center flex-shrink-0 px-4"
          style="padding-left: clamp(1rem, 3vw, 1.5rem); padding-right: clamp(1rem, 3vw, 1.5rem);">
        ...
        <nav class="mt-8 flex-1 px-2 space-y-1"
          style="padding-left: clamp(0.5rem, 2vw, 1rem); padding-right: clamp(0.5rem, 2vw, 1rem);">
        ...
        <div class="flex-shrink-0 flex border-t border-gray-200 p-4">
          <div class="flex items-center">
            ...
            <div class="ml-3">
              ...
              <button @click="$emit('logout')" class="text-xs text-gray-500 hover:text-gray-700 transition-colors">
                Cerrar sesión
              </button>
            </div>
          </div>
      ...
      <div class="relative flex-1 flex flex-col max-w-xs w-full bg-white">
        ...
        <div class="flex-1 h-0 pt-5 pb-4 overflow-y-auto">
          ...
          <nav class="mt-5 px-2 space-y-1">
            <template v-for="item in navigation" :key="item.id">
              <button @click="$emit('navigate', item.id); $emit('toggle-mobile')" :class="[
                item.id === activeSection
                  ? 'bg-primary text-white'
                  : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900',
